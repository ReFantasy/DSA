...
void CBurnCDDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_STATIC_FileListSize, m_FileLIstSize);
	DDX_Control(pDX, IDC_STATIC_MTYPE, cur_media_type);
}
...
BEGIN_MESSAGE_MAP(CBurnCDDlg, CDialog)
	...
	//}}AFX_MSG_MAP
	ON_BN_CLICKED(IDC_EXITMEDIA_BUTTON, &CBurnCDDlg::OnBnClickedExitmediaButton)
	ON_BN_CLICKED(IDC_CLOSEMEDIA_BUTTON, &CBurnCDDlg::OnBnClickedClosemediaButton)
END_MESSAGE_MAP()


// CBurnCDDlg message handlers
constexpr int MAX_LENGTH = 2048; // 字符串buffer长度
...
BOOL CBurnCDDlg::OnInitDialog()
{
	...
	CString IniFileName("./cal.ini");
	CString str_count;
	GetPrivateProfileString(_T("SERIES"), _T("count"), _T("0"), str_count.GetBuffer(MAX_LENGTH), MAX_LENGTH, IniFileName);
	int count  = _ttoi(str_count);
	if (count > 0)
	{
		for (int i = 1; i <= count; i++)
		{
			CString dcm_name = _T("");
			CString dcm_key;
			dcm_key.Format(_T("f%d"), i);
			GetPrivateProfileString(_T("SERIES"), dcm_key, _T("f1"), dcm_name.GetBuffer(MAX_LENGTH), MAX_LENGTH, IniFileName);

			std::string str = CW2A(dcm_name.GetString());
			for (int i = 0; i < str.size(); i++) {
				if (str[i] == '/') str[i] = '\\';
			}

			dcm_name = str.c_str();
			CFileObject* pFileObject = new CFileObject(dcm_name);
...
void CBurnCDDlg::OnCbnSelchangeDeviceCombo()
{
	...
	if (discRecorder != NULL)
	{
		...
		MediaType MT;
		CString media_type = MT.GetMediaTypeString(mediaType);
		cur_media_type.SetWindowText(media_type);
...
void CBurnCDDlg::OnBnClickedBurnButton()
{
	...
	else
	{
		// 未知设备禁止刻录
		/*CString type;
		cur_media_type.GetWindowTextW(type);
		if (type == _T("Unknown Media Type"))
		{
			MessageBox(_T("未知媒体类型，请选择刻录设备！"));
			return;
		}*/
...
void CBurnCDDlg::UpdateCapacity()
{
	...
	if (discRecorder != NULL)
	{
		...
		long t;
		discFormatData.GetInterface()->get_TotalSectorsOnMedia(&t);
		totalSize = (long long)t * 2048L;

		m_maxText.SetWindowText(FileSizeToCString(totalSize));
	}
	//----------

	// 更新界面文件大小
	CString str_file_list_size;
	str_file_list_size = FileSizeToCString(mediaSize);
	m_FileLIstSize.SetWindowText(str_file_list_size);
...
void CBurnCDDlg::OnBnClickedExitmediaButton()
{
	// 弹出光驱
	int selectedIndex = m_deviceComboBox.GetCurSel();
	ASSERT(selectedIndex >= 0);
...
void CBurnCDDlg::OnBnClickedClosemediaButton()
{
	// 关闭光驱
	int selectedIndex = m_deviceComboBox.GetCurSel();
	ASSERT(selectedIndex >= 0);
...
CString FileSizeToCString(ULONGLONG mediaSize)
{
	CString str_file_list_size;
	if (mediaSize < 1024 * 1024) //kb
	{
		str_file_list_size.Format(_T("%.2f KB"), mediaSize / 1024.0);
	}
	else if (mediaSize < 1024 * 1024 * 1024)
	{
		str_file_list_size.Format(_T("%.2f MB"), mediaSize / 1024.0 / 1024.0);
	}
	else
	{
		str_file_list_size.Format(_T("%.2f GB"), mediaSize / 1024.0 / 1024.0 / 1024.0);
	}

	return str_file_list_size;
}

CString MediaType::GetMediaTypeString(int mediaType)
{
	switch (mediaType)
	{
	case IMAPI_MEDIA_TYPE_UNKNOWN:
