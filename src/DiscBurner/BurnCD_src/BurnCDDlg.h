...
protected:
	...
	DECLARE_MESSAGE_MAP()
public:
	afx_msg void OnBnClickedExitmediaButton();
	afx_msg void OnBnClickedClosemediaButton();
	CStatic m_FileLIstSize;
	CStatic cur_media_type;
};


// 输出文件大小字符
CString FileSizeToCString(ULONGLONG file_size);

// 获取媒体类型
class MediaType
{
public:
	bool m_isCdromSupported = false;
	bool m_isDvdSupported = false;
	bool m_isDualLayerDvdSupported = false;
